...
constexpr size_t VALUE_TO_COMPARE_ARRAY_SIZE = 21;
constexpr unsigned char GValueToCompareArray[VALUE_TO_COMPARE_ARRAY_SIZE] =
{
...
int main()
{
	for (size_t ArrayIdx = 0; ArrayIdx < VALUE_TO_COMPARE_ARRAY_SIZE; ++ArrayIdx)
	{
		for (unsigned char Char = MIN_CHAR_VALUE; Char <= MAX_CHAR_VALUE; ++Char)
		{
			const unsigned char MulValue = 0xFB * Char;

			if (GValueToCompareArray[ArrayIdx] == (MulValue & 0xFF))
			{
				printf("%c", Char);
